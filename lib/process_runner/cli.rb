  ...
  class CLI # :nodoc:
    ...
    def parse_options(argv)
      ...
      @parser = OptionParser.new do |o|
        ...
        o.on('-cFILE', '--config FILE', 'Specify a configuration file. Default is config/process_runner.yml in the rails app') do |arg|
      ...
      @parser.banner = 'process_runner [options]'
    ...
    def validate!
      if !File.exist?(options[:require]) ||
        ...
        logger.info 'Please point process runner to a Rails application or a Ruby file'
    ...
    SIGNAL_HANDLERS = {
        ...
        USR1: lambda { |cli|
          ProcessRunner.logger.info 'Received USR1, no longer accepting new work'
          cli.manager.quiet
        },
        TSTP: lambda { |cli|
          ProcessRunner.logger.info 'Received TSTP, no longer accepting new work'
          cli.manager.quiet
        },
        TTIN: lambda { |_cli|
          Thread.list.each do |thread|
            ProcessRunner.logger.warn "Thread TID-#{(thread.object_id ^ ::Process.pid).to_s(36)} #{thread.name}"
            if thread.backtrace
              ProcessRunner.logger.warn thread.backtrace.join("\n")
            else
              ProcessRunner.logger.warn '<no backtrace available>'
            end
          end
        },
    }.freeze
