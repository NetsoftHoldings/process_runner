...
require 'process_balancer/manager'

RSpec.describe ProcessBalancer::Manager do
  let(:base_options) { ProcessBalancer::DEFAULTS }
  let(:job_sets) { [] }
  ...
  def add_worker(identifier, stale: false)
    redis.rpush(ProcessBalancer::PROCESSES_KEY, identifier)
    redis.hmset(identifier, 'beat', Time.now.to_f) unless stale
  end
  ...
  describe '#initialize' do
    ...
    it 'creates the watcher' do
      expect(watchers[:test_job]).to be_a(ProcessBalancer::Watcher)
    end
  ...
  describe '#workers_for_job' do
    ...
    it 'calls the ProcessBalancer.scheduled_workers' do
      expect(ProcessBalancer).to receive(:scheduled_workers).with(job_id)
  ...
  describe '#heartbeat' do
    ...
    describe 'update_process_index - managing the process index' do
      ...
      context 'when there is a stale worker' do
        before do
          add_worker('worker 1')
          add_worker('worker 2', stale: true)
        end
    ...
    describe 'update_jobs - managing the watchers' do
      let(:job_sets) { [{id: :test_job, class: 'MyClass'}] }
      let(:watcher) { instance_double(ProcessBalancer::Watcher, update_worker_config: nil, stats: {}) }
      let(:workers_key) { "#{instance.identity}:workers" }

      before do
        allow(ProcessBalancer::Watcher).to receive(:new).and_return(watcher)
      end
    ...
    context 'when any error occurs' do
      ...
      it 'logs it' do
        expect(ProcessBalancer.logger).to receive(:error).with(/heartbeat: Something/)
