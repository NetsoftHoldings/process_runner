...
require 'process_balancer/lock/simple_redis'

RSpec.describe ProcessBalancer::Lock::SimpleRedis do
  subject { instance.perform }

  let(:job_class) do
    Class.new(ProcessBalancer::Base) do
      include ProcessBalancer::Lock::SimpleRedis
  ...
  context 'when the lock is obtained' do
    it 'sets a lock in redis' do
      expect(redis).to receive(:set).with('lock_my_job_1', ProcessBalancer.identity, hash_including(nx: true)).and_call_original
      expect(redis).to receive(:set).with('lock_my_job_1', ProcessBalancer.identity, hash_not_including(nx: true)).and_call_original
  ...
  context 'when the lock is not obtained' do
    ...
    it 'sleeps between each attempt' do
      handler = described_class::LockHandler.new('lock_my_job_1', ProcessBalancer.identity, 30)
