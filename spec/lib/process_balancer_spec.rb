...
require 'process_balancer'

RSpec.describe ProcessBalancer do
  worker_count_key = ProcessBalancer::WORKER_COUNT_KEY
  processes_key = ProcessBalancer::PROCESSES_KEY

  it 'has a version number' do
    expect(ProcessBalancer::VERSION).to_not be nil
  end
  ...
  describe '.redis_pool' do
    ...
    it 'calls calls RedisConnection to create a pool' do
      ...
      expect(ProcessBalancer::RedisConnection).to receive(:create).with(test_key: true)
