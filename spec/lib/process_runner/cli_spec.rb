...
require 'process_runner/cli'

RSpec.describe ProcessRunner::CLI do
  ...
  describe '#parse' do
    ...
    context 'with verbose arg' do
      ...
      it 'sets the verbosity option' do
        expect { subject }.to change { ProcessRunner.options[:verbose] }.to(true)
      end
    ...
    describe 'locating the config file' do
      context 'when no args specified' do
        context 'when that default config/process_runner.yml file exists' do
          before do
            FileUtils.touch('./config/process_runner.yml')
          end

          it 'loads that config file' do
            expect(instance).to receive(:parse_config).with('./config/process_runner.yml').and_return({})
      ...
      context 'when the require arg is specified' do
        ...
        context 'and is a directory' do
          ...
          context 'when config file exists in that app root' do
            before do
              FileUtils.touch('app/config/process_runner.yml')
            end

            it 'loads that config file' do
              expect(instance).to receive(:parse_config).with('app/config/process_runner.yml').and_return({})
        ...
        context 'and is a file' do
          ...
          context 'when ./config/process_runner.yml file exists' do
            before do
              FileUtils.touch('./config/process_runner.yml')
            end
    ...
    context 'with a configuration file' do
      ...
      before do
        File.write('./config/process_runner.yml', contents)
      end
      ...
      context 'with a simple config file' do
        ...
        it 'merges in the config options' do
          expect { subject }.to change { ProcessRunner.options[:max_threads] }.to(5)
        end
      end

      context 'with overridden ENV config' do
        let(:contents) do
          <<~YAML
            global:
              max_threads: 5
        ...
        it 'merges in the config options' do
          expect { subject }.to change { ProcessRunner.options[:max_threads] }.to(3)
        end
      end
      ...
      context 'with jobs defined' do
        ...
        it 'defines the job sets' do
          expect { subject }
              .to change { ProcessRunner.options[:job_sets] }
    ...
    describe 'parameter validation' do
      shared_examples 'fail require validation' do
        it 'logs info' do
          ...
          expect(ProcessRunner.logger).to receive(:info).at_least(3).times
  ...
  describe '#run' do
    ...
    let(:manager) { instance_double(ProcessRunner::Manager, run: true, stop: true) }

    before do
      allow(ProcessRunner::Manager).to receive(:new).and_return(manager)
    ...
    it 'creates a manager instance' do
      expect(ProcessRunner::Manager).to receive(:new).with(ProcessRunner.options).and_return(manager)
  ...
  describe '#boot_system' do
    ...
    context 'when require option is a file' do
      before do
        ProcessRunner.options[:require] = 'boot.rb'
      end
  ...
  describe '::SIGNAL_HANDLERS' do
    ...
    let(:manager) { instance_double(ProcessRunner::Manager) }
    ...
    describe 'unknown' do
      ...
      it 'logs an info that no handler was found' do
        expect(ProcessRunner.logger).to receive(:info).with(/No signal handler/)
